...
Create-Diagram-Thumbnail `
    -outputPath "static/images/blog/column-03-tic-pangyo.jpg"     -categoryTag "판교 틱장애·소아신경"     -mainTitle "판교 틱장애 원인과 치료법"     -subTitle "눈 깜빡임·음음 소리, 억지로 참게 하면 안 되는 이유"     -parentBoxText "두뇌 기저핵 (Basal Ganglia)"     -parentBoxSub "운동 제어 및 불필요한 신호 필터링 중추"     -leftBoxText "기저핵 기능 미성숙"     -leftBoxSub "불필요한 동작 억제 실패`n`n▶ 눈 깜빡임·고개 털기`n▶ 킁킁·음음 음성틱 발생"     -rightBoxText "해아림 두뇌 밸런스 케어"     -rightBoxSub "뇌 신경계 흥분 안정`n`n▶ 뇌파·체질 맞춤 한약`n▶ 자율신경 조절 & 침구치료"     -themeColorLeft "#F87171"     -themeColorRight "#2DD4BF"

# 2. Column 4: [수지 불면증]
Create-Diagram-Thumbnail `
    -outputPath "static/images/blog/column-04-sleep-suji.jpg"     -categoryTag "수지 불면증·수면클리닉"     -mainTitle "수지 불면증 원인과 회복"     -subTitle "밤마다 뒤척이는 뇌의 과각성, 자연 수면 리듬 되찾기"     -parentBoxText "수면 조절 중추 (Sleep Center)"     -parentBoxSub "시상하부 및 멜라토닌 분비 조절 시스템"     -leftBoxText "뇌 신경계 과각성 상태"     -leftBoxSub "교감신경 항진·긴장 지속`n`n▶ 30분 이상 입면 장애`n▶ 수면 유지 장애·조기 각성"     -rightBoxText "해아림 안심(安心) 수면치료"     -rightBoxSub "뇌 과열 진정·기혈 순환`n`n▶ 수면유도 맞춤 한약`n▶ 심신 안정 침구 & 생활요법"     -themeColorLeft "#FB923C"     -themeColorRight "#38BDF8"
