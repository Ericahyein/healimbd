...
Create-Diagram-Thumbnail \`
    ...
    -categoryTag "판교 틱장애·소아신경" \
    -mainTitle "판교 틱장애 원인과 치료법" \
...
Create-Diagram-Thumbnail \`
    ...
    -categoryTag "수지 불면증·수면클리닉" \
    -mainTitle "수지 불면증 원인과 회복" \
...
fs.writeFileSync('scratch/gen_regional_thumbs.ps1', '\ufeff' + psCode, 'utf-8');
console.log('Saved scratch/gen_regional_thumbs.ps1');
try {
  const res = execSync('powershell.exe -ExecutionPolicy Bypass -File scratch/gen_regional_thumbs.ps1', { encoding: 'utf-8' });
  console.log(res);
} catch (e) {
