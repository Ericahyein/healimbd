...
// Replace the Inquiry section with Firebase integration
...
if (sIdx !== -1 && eIdx !== -1) {
  const firebaseInquiryEngine = `let currentInquiryFilter = 'all';
...
`;
  ...
  mainJs = mainJs.substring(0, sIdx) + firebaseInquiryEngine + mainJs.substring(eIdx);

  // Also update handleInquirySubmit, handleDoctorReplySubmit, handleAdminDeleteInquiry with Cloud Firestore syncing
  mainJs = mainJs.replace(/function handleInquirySubmit\(e\) \{[\s\S]*?renderInquiryList\(\);\s*\}/, `async function handleInquirySubmit(e) {
  e.preventDefault();
  ...
  mainJs = mainJs.replace(/function handleDoctorReplySubmit\(e\) \{[\s\S]*?renderInquiryList\(\);\s*\}/, `async function handleDoctorReplySubmit(e) {
  e.preventDefault();
  ...
  mainJs = mainJs.replace(/function handleAdminDeleteInquiry\(\) \{[\s\S]*?renderInquiryList\(\);\s*\}/, `async function handleAdminDeleteInquiry() {
  if (!currentOpenedInquiryId) return;
  ...
  console.log('Successfully added Firebase Cloud Firestore Real-Time Database Engine to assets/js/main.js!');
} else {
