...
// Safe sentences that MUST PASS without false positive
...
safeSampleSentences.forEach((sentence, sIdx) => {
  const mockArticle = {
...
환자분들께서 자주 문의하시는 내용입니다.
${sentence}

## 2. 주요 증상 및 배경
갑작스러운 두근거림과 신체 반응입니다.

## 3. 감별 진단 포인트
신체 증상과 공황의 구별입니다.
[안내 링크](/blog/bundang-panic-disorder-treatment-guide/)

## 4. 한방 관리 관점
체질 맞춤 탕약과 침구 치료입니다.
[자율신경 안내](/blog/bundang-autonomic-nervous-system-recovery/)
...
`,
  ...
  const res = validateArticleContent(mockArticle);
  const medErr = res.errors.filter(e => e.includes('약물 중단 권고 금지'));
  assert.strictEqual(medErr.length, 0, `Safe sentence [${sIdx}] was falsely blocked: "${sentence}" (Errors: ${medErr.join(', ')})`);
});
console.log('✅ [Test 5-1 Passed] All safe medication warning sentences passed validation with 0 false positives.');

// Dangerous stop-recommending sentences that MUST BE BLOCKED
...
dangerousSentences.forEach((sentence, dIdx) => {
  const mockArticle = {
    title: '[분당 공황장애] 갑자기 숨이 차고 심장이 빨라질 때 먼저 살펴볼 부분',
    summary: '분당 지역 주민들을 위한 공황장애 갑작스러운 호흡곤란 증상 관리 가이드입니다.',
    body: `
...
`,
  ...
  const res = validateArticleContent(mockArticle);
  const medErr = res.errors.filter(e => e.includes('약물 중단 권고 금지'));
  assert.ok(medErr.length > 0, `Dangerous sentence [${dIdx}] was NOT blocked: "${sentence}"`);
});
console.log('✅ [Test 5-2 Passed] Dangerous medication stop-recommending sentences were strictly blocked.');
