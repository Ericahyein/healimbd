...
### 5.1 메인 히어로 섹션 & 네이비 정보 바 (`layouts/index.html`)
* **원장 진료실 배경 사진 & 톤다운 오버레이**:
  * 배경 이미지: `static/images/hero-doctor.jpg` (원장 진료 및 QEEG 뇌파 검사 배경)
  * 다크 네이비 그라데이션 오버레이(`linear-gradient(rgba(15, 23, 42, 0.70), rgba(15, 23, 42, 0.94))`)를 적용하여 텍스트 시인성과 고급스러운 무드 극대화
* **메인 카피라이트 (화이트 텍스트)**:
  * **굵은 글씨 (H1)**: `몸과 뇌, 마음을 함께 살피는 신경정신과 한방 진료`
  * **얇은 서브카피 (Light)**: `공황·불안·불면·자율신경실조증부터 틱장애·ADHD까지, 증상 너머의 원인을 살피고 개인에 맞는 회복 과정을 설계합니다.`
* **네이비 정보 바 (`.navy-info-bar`)**:
  * 히어로 바로 아래에 배치되어 야간진료/공휴일 진료시간, 정자역 3번출구 위치, 대표전화(`031-716-8575`)를 직관적으로 전달

### 5.2 상단 헤더 & GNB (`layouts/partials/header.html`)
