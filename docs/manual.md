...
### 5.1 메인 히어로 섹션 (`layouts/index.html`)
* **원장 진료실 배경 사진 & 톤다운 오버레이**:
  * 배경 이미지: `static/images/hero-doctor.jpg` (원장 진료 및 QEEG 뇌파 검사 배경)
  * 다크 네이비 그라데이션 오버레이(`linear-gradient(rgba(15, 23, 42, 0.45), rgba(15, 23, 42, 0.82))`)를 적용하여 텍스트 시인성과 원장님 진료실 사진의 신뢰감 있는 무드 극대화
* **메인 카피라이트 (화이트 텍스트)**:
  * **굵은 글씨 (H1)**: `몸과 뇌, 마음을 함께 살피는 신경정신과 한방 진료`
  * **얇은 서브카피 (Light)**: `공황·불안·불면·자율신경실조증부터 틱장애·ADHD까지, 증상 너머의 원인을 살피고 개인에 맞는 회복 과정을 설계합니다.`

### 5.2 치료후기 사진 게시판 시스템 (`content/reviews/`, `layouts/reviews/`)
* **개요**: 한의원에서 직접 환자 자필 후기 사진과 상세 회복 글을 등록하고 관리할 수 있는 독립형 게시판 시스템.
* **게시판 구성**:
  * **메인 홈 갤러리 (`#reviews`)**: 대표 후기 4건이 사진 썸네일, 증상 태그, 별점, 요약글과 함께 2열 카드 그리드로 표출.
  * **전체 치료후기 게시판 (`/reviews/`)**: 질환별 카테고리 필터 탭(전체, 성인 공황·불안, 수면·자율신경, 소아 틱장애, 소아청소년 ADHD)을 통해 전체 후기 탐색 가능.
  * **개별 후기 상세 페이지 (`/reviews/글제목/`)**: 후기 원본 사진 크게 보기, 환자 자필 내용, 원장 치료 코멘트, 상담 예약 바로가기 제공.

### 5.3 상단 헤더 & GNB (`layouts/partials/header.html`)
* **로고 & 텍스트**: 공식 로고 이미지(`logo.png`)와 브랜드 색상(`color: #515658`) 적용
* **5대 핵심 메뉴**:
  1. `치료후기` (`#reviews` 또는 `/reviews/`): 환자 자필 후기 사진 게시판 연동
...
### 5.4 상담 및 예약 팝업 모달 (`inquiry-modal`)
...
## 6. 치료후기 새 글 & 사진 등록 방법 (운영 매뉴얼)

치료후기를 새로 작성하시려면 아래 순서대로 파일과 사진을 넣으시면 자동으로 홈페이지 및 게시판에 반영됩니다:

### 1단계: 후기 사진 파일 저장
* 촬영한 자필 후기 사진(jpg 또는 png)을 `static/images/reviews/` 폴더에 복사합니다.
* 예시 파일명: `static/images/reviews/review-panic-2026.jpg`

### 2단계: 새 후기 마크다운(`.md`) 파일 생성
`content/reviews/` 폴더 안에 새 파일(예: `panic-recovery-02.md`)을 생성하고 아래 형식으로 작성합니다:

```markdown
---
title: "치료후기 제목을 여기에 입력하세요"
date: 2026-08-25
author: "홍O동 님"
patient_info: "30대 직장인"
category: "adult"                # adult(공황불안), sleep(수면자율신경), child(소아틱), adhd(소아ADHD) 중 택1
category_name: "성인 공황·불안"
rating: 5
image: "images/reviews/review-panic-2026.jpg"   # 1단계에서 넣은 사진 경로
summary: "홈페이지 목록에 간단하게 노출될 2~3줄 요약글을 작성합니다."
---

### 💬 환자 자필 치료후기 내용
> "환자분이 남겨주신 소중한 자필 후기 본문을 여기에 입력합니다."

### 🌿 손지웅 대표원장의 치료 코멘트
* **주요 증상**: 호소 증상 및 진단 내용
* **치료 과정**: 한약 처방, 침구치료, 뉴로피드백 등 적용 치료법
* **경과**: 치료 후 호전 상태 및 현재 일상 복귀 상황
```

### 3단계: GitHub 업로드
* 바탕화면의 `해아림_깃허브업로드.bat` 파일을 더블클릭하면 1-클릭으로 즉시 온라인 배포 완료!
