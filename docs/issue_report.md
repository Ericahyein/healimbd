...
| **ISSUE-11** | 기능 확장/게시판 | 네이비 정보 바 제거 및 사진과 함께 직접 글을 등록하는 독립형 치료후기 게시판 구축 | **해결 완료** | 2026-08-25 |
...
* **해결 조치**:
  ...
  - 헤더, 톤다운 진료실 사진 히어로, 자가진단, 클리닉, 프로세스, 칼럼, FAQ, 오시는 길, 푸터까지 모든 섹션이 100% 무결하게 HTML로 직접 컴파일되도록 조치 완료.

---

### 📌 ISSUE-11: 네이비 정보 바 제거 및 사진과 함께 직접 글을 등록하는 독립형 치료후기 게시판 구축
* **요청 사항**:
  1. 히어로 섹션 하단에 노출되던 네이비색 진료안내 바 삭제.
  2. 한의원에서 직접 자필 후기 사진과 상세 회복 글을 작성하여 등록할 수 있는 독립형 치료후기 게시판 구축.
* **해결 조치**:
  1. `layouts/index.html` 및 관련 스타일에서 `.navy-info-bar` 섹션 마크업 및 불필요 레이아웃 완전 제거.
  2. Hugo 콘텐츠 기반의 독립형 치료후기 시스템(`content/reviews/`) 구축:
     - `content/reviews/*.md`: 마크다운 기반의 구조화된 후기 글 작성 템플릿(사진 경로, 제목, 작성자, 카테고리 태그, 별점, 원장 코멘트).
     - `layouts/reviews/list.html`: 카테고리 탭 필터링(전체, 성인공황, 수면, 소아틱, ADHD)이 지원되는 치료후기 게시판 목록 페이지(`/reviews/`).
     - `layouts/reviews/single.html`: 자필 후기 사진 원본 및 원장 진료 소견, 예약 바로가기를 포함한 후기 상세 뷰.
  3. 메인 홈페이지의 `#reviews` 섹션을 최신 자필 후기 사진 카드 갤러리로 업그레이드하고, `docs/manual.md`에 후기 사진/글 작성 매뉴얼 추가 완료.
